...
#I don't like how the groupings are here. Want to measure by the variables:

# ------------------------------------------------------------
# Purpose:
# Visualize probability of automation by education requirement
# using binary education fields, while incorporating total
# employment weights so that the distribution reflects worker
# exposure rather than equal-weighted occupations.
#
# NOTE:
# geom_boxplot() does not natively support weights. To address
# this, occupations are expanded proportional to total
# employment (scaled to remain computationally feasible).
# ------------------------------------------------------------

# 1. Prepare long-format education data
edu_long <- prob_auto_final %>%
  select(
    p_automation,
    TOT_EMP,
    no_ed_requirement,
  ...
  pivot_longer(
    cols = -c(p_automation, TOT_EMP),
    names_to = "education_category",
    values_to = "flag"
  ) %>%
  filter(flag == 1)

# 2. Relabel education categories for readability
edu_long <- edu_long %>%
  mutate(
    education_category = recode(
      education_category,
      no_ed_requirement = "No Education Requirement",
      hs_diploma_or_equiv = "High School Diploma or Equivalent",
      post_secondary_no_degree = "Postsecondary — No Degree",
...
# ------------------------------------------------------------
# 3. Incorporate employment weights
#
# Each occupation is expanded proportional to total employment.
# The scaling factor (e.g., / 10,000) preserves relative worker
# representation while keeping the dataset manageable.
# ------------------------------------------------------------

edu_long_weighted <- edu_long %>%
  mutate(weight = round(TOT_EMP / 10000)) %>%  # scaling factor
  filter(weight > 0) %>%
  uncount(weight)

# 4. Order education categories by median automation risk
edu_long_weighted <- edu_long_weighted %>%
  mutate(
    education_category = reorder(
      education_category,
      p_automation,
      FUN = median
    )
...
# 5. Plot
ggplot(edu_long_weighted,
       aes(x = education_category, y = p_automation)) +
  geom_boxplot(
    outlier.alpha = 0.3,
    fill = "white",
    color = "grey30"
  ...
  coord_flip() +
  labs(
    title = "Probability of Automation by Education Requirement",
    subtitle = "Distribution weighted by total occupational employment",
...
ggplot(prob_auto_final, aes(x = routineness, y = p_automation)) +
  ...
  labs(
    x = "Task Routineness",
    y = "Probability of Automation",
    title = "Task Routineness and Automation Risk"
  ) +
  scale_y_continuous(limits = c(0, 1)) +
  theme_minimal(base_size = 12)
